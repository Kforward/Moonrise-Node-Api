...
- 已补充 key wrap 托管策略：后续密码锁、恢复密钥和可信设备换机只保存被 KEK 包裹后的 DEK，不保存明文密钥或恢复短语。
...
- 后续实现恢复能力时，按 `backend_ai_docs/03_api_design.md` 与 `backend_ai_docs/04_database_design.md` 中的 key wrap 草案补接口、表结构和审计日志。
...
- key wrap 目前仍是设计草案，未创建真实数据库表或公开接口。
