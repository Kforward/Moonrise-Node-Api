...
18. 已完成：开放 `app` 应用级接口，支持用户轻量偏好读取/幂等更新、应用更新日志列表/详情，并接入 `user_app_preferences.update` 同步日志。
19. 下一步：继续扩展 PostgreSQL 仓储边界场景，以及 Redis/多实例限流适配。
...
当前主链路已可供前端基于内存仓储联调，以下事项属于后续补强或生产化工作，不阻塞基础功能使用：

- PostgreSQL 仓储边界测试仍需扩展：当前已有可选 `npm run test:postgres` 入口，但默认需要显式测试库和 `RUN_POSTGRES_TESTS=1` 才会真实执行。
- Redis/多实例限流适配仍未落地：当前登录、刷新 token、备份创建等接口使用进程内固定窗口限流，横向扩容后需要替换为共享存储。
- 安全失败路径审计仍需补齐：刷新 token 失败过多、批量同步失败或冲突、解密失败等场景应进一步写入审计日志。
- key wrap 恢复能力仍需落地：当前已补充 DEK/KEK、恢复密钥、可信设备和不可恢复 reset 的设计草案，尚未新增真实表、接口、审计测试和前端联调。
- 分页契约与生产实现仍需收敛：设计草案期望游标包含 `createdAt + id`，当前实现主要使用记录 ID 作为 `nextCursor`，PostgreSQL 列表查询也还有全量查询后内存切页的优化空间。
- 事务一致性仍需增强：业务写入、同步日志、审计日志和幂等快照目前按步骤完成，后续 PostgreSQL 模式应尽量收敛到同一事务边界。
