...
当前默认使用 `DATABASE_DRIVER=memory`，便于前端本地联调不依赖 PostgreSQL。PostgreSQL/Drizzle 基础设施、schema 和初始迁移已接入，`auth` 已具备内存/PostgreSQL 双仓储实现，认证审计日志可在 PostgreSQL 模式写入 `audit_logs`。后续会逐步把 `users / cycle / sync` 仓储从内存适配器切到持久化适配器。
